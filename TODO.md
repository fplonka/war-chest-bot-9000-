...
- Encode `wp_v2_triggered` in the public replay row. It changes whether a later Warrior Priest V2 maneuver triggers, so omitting it aliases states with different transitions in search interning and the value network.
- Revisit belief-conditioned root priors without restoring the uniform-belief bug. The correctness fix remains on `fix/root-prior-belief` at `f85873e`; its isolated 60-minute bot lost 173–224–3 to `combined_9h_20260828.s2` (paired 23–50, p=0.0021, about -48 Elo). Explain or recover this strength regression before merging it.
