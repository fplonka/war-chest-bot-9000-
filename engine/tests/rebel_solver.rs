...
/// What half precision would cost the search, in the units that matter.
///
/// Tensor cores are a precondition for 150 solves a second -- a solve costs
/// 41 ms of card time and the target allows 13.3 -- and the obstacle has
/// always been stated numerically: half precision moves the board vector
/// 5.39e-2 against a 1e-3 parity bound. Fifty-four times over sounds fatal.
///
/// But a parity bound is a statement about agreeing with the CPU reference,
/// not about searching well, and those are different questions. This asks the
/// second one: the same roots, the same iterations, the same stream, leaf
/// values rounded to ten mantissa bits before CFR reads them, and `nash_conv`
/// on what the solve arrives at.
///
/// If exploitability is unmoved, the 5.39e-2 is a tolerance to re-justify
/// rather than a wall, and a tiled half-precision GEMM is worth building. If
/// it is not, that work is dead and the device has to find its 3.1x elsewhere.
#[test]
fn half_precision_leaves_cost_the_search_this_much() {
    let nets = Nets::default();
    let (mut sum, mut checked) = ([0.0f64; 2], 0usize);
    for seed in 0..20000u64 {
        let Some(s) = micro_position(seed, 60 + (seed as usize % 120), 3) else {
            continue;
        };
        ...
        for (i, half) in [false, true].into_iter().enumerate() {
            let cfg = Cfg {
                nodes: 3_000,
                iters: 32,
                expand: 4,
    ...
    assert!(checked >= 30, "only {checked} positions were solvable");
    let (single, half) = (sum[0] / checked as f64, sum[1] / checked as f64);
    println!(
        "NashConv over {checked} roots: single {single:.5}  half {half:.5}  \
         ratio {:.3}",
        half / single.max(1e-12)
    );
    // Not a bound anyone chose in advance -- it is here so the number is
    // recorded and a change to the network has to look at it again.
    assert!(
        half.is_finite() && single.is_finite(),
        "exploitability stopped being finite: {single} {half}"
    );
}
