...
/// A layout that does not fit the largest class is rejected.
#[test]
fn a_layout_that_misses_the_largest_class_does_not_fit() {
    let mut lens = warchest::slab::SHAPE;
    assert!(warchest::slab::fits(&lens));
    lens[1] = 1 << 30;
    assert!(!warchest::slab::fits(&lens));
}

#[test]
fn a_bigger_card_carves_more_slabs() {
    let small = warchest::slab::mix(3 << 30, 10 << 20, 2);
    let big = warchest::slab::mix(20 << 30, 10 << 20, 2);
    assert!(big.iter().sum::<usize>() > small.iter().sum::<usize>());
    assert!(small[0] >= 8 && small[5] >= 2);
}
