...
fn main() {
    ...
    while positions < want {
        ...
        // Leaf values are hand-keyed, so within a subgame two such configs are
        // told apart only by a draw node. Everywhere else their regrets are
        // identical and CFR hands them the same distribution over actions --
        // i.e. the agent cannot act on its own bag even though it knows it.
        {
        ...
        for (di, &d) in DEPTHS.iter().enumerate() {
            let cfg = Cfg { depth: d, iters, average: false };
            let mut sv = Solver::new(&s, &ctx, &nets, cfg, bel.clone());
            sv.multistep(iters);
            for p in 0..2 {
                let v = sv.root_values(p);
