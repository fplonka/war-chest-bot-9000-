...
def health(run):
    ...
    if last:
        bits.append(f"t={last.get('t', 0) / 60:.0f}m")
...
def href(run):
    for name in ("report.html", "train.log", "log.json"):
        if os.path.isfile(os.path.join(RUNS, run, name)):
            return name
    return "log.json"
...
def main():
    ...
    for n in names:
        ...
        sub = health(n)
        items.append(
            f'<li><a href="{html.escape(n)}/{href(n)}">{html.escape(n)}</a>'
